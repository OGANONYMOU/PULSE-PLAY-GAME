...
const BLANK: Omit<Game, 'id' | 'created_at'> = {
  name: '', description: '', icon: '🎮', image_url: null, badge: null,
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Logo Image URL (optional)">
              <Input value={form.image_url ?? ''} onChange={e => set('image_url', e.target.value || null)} placeholder="https://example.com/game-logo.png" className={inputCls} />
              {form.image_url && (
                <img src={form.image_url} alt="preview" className="mt-2 w-14 h-14 rounded-xl object-cover border border-white/10" />
              )}
            </Field>
          </div>
...
function GameRow(p: { game: Game; index: number; onEdit: () => void; onDelete: () => void }): React.ReactElement {
  ...
  return (
    ...
    >
      {/* Icon / Logo */}
      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-white/8 to-white/4 border border-white/10 flex items-center justify-center text-2xl flex-shrink-0 overflow-hidden">
        {g.image_url
          ? <img src={g.image_url} alt={g.name} className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display='none'; }} />
          : g.icon}
      </div>
