...
function roleBadge(role: string): string {
  if (role === 'ADMIN') return 'bg-red-500/20 text-red-400 border-red-500/40';
  if (role === 'MODERATOR') return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/40';
...
function AvatarDisplay(p: { url: string | null; username: string }): React.ReactElement {
  const letter = p.username[0] ? p.username[0].toUpperCase() : 'U';
  return p.url ? (
    <img src={p.url} alt={p.username} className="w-full h-full object-cover rounded-full" />
  ) : (
    <div className="w-full h-full rounded-full flex items-center justify-center bg-gradient-to-br from-cyan-500 to-purple-600 font-orbitron font-bold text-white text-2xl sm:text-3xl">
      {letter}
    </div>
  );
}
function AdminBadge(p: { show: boolean }): React.ReactElement {
...
function PrivateInfo(p: { show: boolean; label: string; value: string | null }): React.ReactElement {
  return p.show && p.value ? (
    <div className="flex items-center gap-2 text-xs text-white/50">
      <span className="text-white/30">{p.label}:</span><span>{p.value}</span>
    </div>
  ) : <span />;
}
function BioText(p: { show: boolean; bio: string | null; isOwn: boolean }): React.ReactElement {
  ...
  return p.show ? <p className="text-white/60 text-sm leading-relaxed mt-3 max-w-2xl">{text}</p> : <span />;
}
function TwitterLink(p: { username: string | null }): React.ReactElement {
  return p.username ? (
    <a href={'https://twitter.com/' + p.username} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 hover:bg-cyan-500/10 border border-white/10 hover:border-cyan-500/30 text-xs text-white/60 hover:text-cyan-400 transition-all">
      <Twitter className="w-3.5 h-3.5" /><span>{'@' + p.username}</span><ExternalLink className="w-3 h-3 opacity-50" />
    </a>
  ) : <span />;
}
function DiscordLink(p: { username: string | null }): React.ReactElement {
  return p.username ? (
    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white/60">
      <MessageSquare className="w-3.5 h-3.5" /><span>{p.username}</span>
    </div>
  ) : <span />;
}
function SocialRow(p: { show: boolean; twitter: string | null; discord: string | null }): React.ReactElement {
  return p.show && (p.twitter || p.discord) ? (
    <div className="flex items-center gap-2 mt-4 flex-wrap">
      ...
      <DiscordLink username={p.discord} />
    </div>
  ) : <span />;
}
...
function BannerBg(p: { url: string | null }): React.ReactElement {
  return p.url ? <img src={p.url} alt="banner" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-r from-cyan-900/40 via-purple-900/40 to-pink-900/40" />;
}
function BannerUploadBtn(p: { show: boolean; uploading: boolean; onClick: () => void }): React.ReactElement {
  return p.show ? (
    <button onClick={p.onClick} disabled={p.uploading} className="absolute bottom-3 right-3 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/60 hover:bg-black/80 border border-white/20 text-white text-xs transition-all">
      <Camera className="w-3.5 h-3.5" />{p.uploading ? 'Uploading...' : 'Change Banner'}
    </button>
  ) : <span />;
}
function AvatarUploadBtn(p: { show: boolean; uploading: boolean; onClick: () => void }): React.ReactElement {
  return p.show ? (
    <button onClick={p.onClick} disabled={p.uploading} className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-cyan-500 hover:bg-cyan-400 flex items-center justify-center border-2 border-background transition-all">
      <Camera className="w-3.5 h-3.5 text-white" />
    </button>
  ) : <span />;
}
function EditPanel(p: { show: boolean; form: EditForm; saving: boolean; onChange: (f: EditForm) => void; onSave: () => void; onCancel: () => void }): React.ReactElement {
  return p.show ? (
    <div className="mt-6 p-5 rounded-xl bg-white/5 border border-white/10 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><label className="text-xs text-white/50 block mb-1.5">Discord</label><Input value={p.form.discord_username} onChange={(e) => p.onChange({ ...p.form, discord_username: e.target.value })} className="bg-white/5 border-white/10 text-white placeholder:text-white/30 h-9 text-sm" placeholder="user#0000" /></div>
      </div>
      <div className="flex gap-2 justify-end pt-1">
        <Button variant="ghost" size="sm" onClick={p.onCancel} disabled={p.saving} className="text-white/60 hover:text-white hover:bg-white/10 h-8 text-xs">Cancel</Button>
        <Button size="sm" onClick={p.onSave} disabled={p.saving} className="bg-gradient-to-r from-cyan-500 to-purple-600 text-white h-8 text-xs font-bold">
          <Save className="w-3.5 h-3.5 mr-1.5" />{p.saving ? 'Saving...' : 'Save'}
        </Button>
      </div>
    </div>
  ) : <span />;
        ...
        {p.posts.map((post) => (
          <div key={post.id} className="p-5 rounded-xl bg-white/5 border border-white/10 hover:border-cyan-500/30 transition-all">
            <div className="flex items-start justify-between gap-3 mb-2">
              ...
              <span className={'text-xs px-2 py-0.5 rounded-full flex-shrink-0 ' + tagClass(post.tag)}>{post.tag}</span>
            </div>
...
export function Profile(): React.ReactElement {
  ...
  const [editForm, setEditForm] = useState<EditForm>({ first_name: '', last_name: '', bio: '', discord_username: '', twitter_username: '' });
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      if (owning) {
        ...
        } else { setPageLoading(false); return; }
      } else {
  ...
  const handleSave = async () => {
    ...
    else { toast.success('Profile updated!'); setProfile((prev) => prev ? { ...prev, ...editForm } : prev); setIsEditing(false); }
    setIsSaving(false);
  };
  ...
  const uploadFile = async (file: File, suffix: string, field: 'avatar_url' | 'banner_url', setUploading: (v: boolean) => void) => {
    ...
    const { error: updErr } = await updateProfile({ [field]: urlData.publicUrl });
    if (updErr) { toast.error('Save failed.'); } else {
      setProfile((prev) => prev ? { ...prev, [field]: urlData.publicUrl } : prev);
    ...
    return (
      <div className="min-h-screen pt-24">
        <div className="max-w-4xl mx-auto px-6 space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3">{[1,2,3,4].map((i) => <div key={i} className="h-24 rounded-xl bg-white/5 animate-pulse" />)}</div>
    ...
    return (
      <div className="min-h-screen pt-24 flex items-center justify-center px-6">
        <div className="text-center p-10 rounded-2xl bg-white/5 border border-white/10 max-w-md w-full">
          ...
          <Button onClick={loadProfile} className="bg-gradient-to-r from-cyan-500 to-purple-600 text-white">Try Again</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen pt-24 flex items-center justify-center px-6">
        <div className="text-center p-10 rounded-2xl bg-white/5 border border-white/10 max-w-md w-full">
          ...
          <p className="text-white/50 text-sm mb-6">This profile does not exist.</p>
          <Button asChild variant="outline" className="border-white/20 text-white hover:bg-white/10"><Link to="/">Back to Home</Link></Button>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div className="relative -mt-16 mx-2 sm:mx-0 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl p-5 sm:p-6 mb-4">
          <div className="flex flex-col sm:flex-row items-start gap-5">
            <div className="relative flex-shrink-0 -mt-14 sm:-mt-16">
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-4 border-black/40">
            ...
            <div className="flex-1 min-w-0 pt-1 sm:pt-0">
              ...
              <div className="flex items-center gap-1.5 text-xs text-white/40 mt-1">
                <Calendar className="w-3 h-3" /><span>Joined {joinedDate}</span>
              </div>
        ...
        <div className="rounded-2xl bg-white/5 border border-white/10 overflow-hidden">
          <div className="flex border-b border-white/10">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={'flex items-center gap-1.5 sm:gap-2 px-2 sm:px-5 py-3 sm:py-3.5 text-xs sm:text-sm font-medium transition-all flex-1 justify-center min-h-[44px] ' + (activeTab === tab.id ? 'text-cyan-400 border-b-2 border-cyan-400 bg-cyan-500/5' : 'text-white/40 hover:text-white/70 hover:bg-white/5')}>
          ...
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-orbitron font-bold text-sm text-white/80">{activeTab === 'posts' ? postsLabel : activeTab === 'tournaments' ? 'Tournament History' : 'Achievements'}</h2>
              {activeTab === 'posts' && isOwnProfile ? (
                <Button asChild size="sm" className="h-7 text-xs bg-gradient-to-r from-cyan-500 to-purple-600 text-white"><Link to="/community">New Post</Link></Button>
              ) : null}
            </div>
